...
def artist_add_youtube(request, artist_pk):
    ...
    # request.POST로 전달된 youtube_id, title, url_thumbnail
    #   ArtistYouTube를 Artist의 youtube_videos에 추가

    # 여기로 데이터를 전달하는 form을
    #    artis_detail에 구현
    # 처리 완료 후, 'next'값이 던달되었으면 그 위치로
    # 아니면 artist_detail로 이동

    # artist_detail에 저장되어있는 youtube_videos목록을 출력

    artist = get_object_or_404(Artist, pk=artist_pk)
    artist.youtube_videos.get_or_create(
